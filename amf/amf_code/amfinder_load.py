...
class CustomNormalisedDataset(Dataset):
    ...
    y: Iterable of labels corresponding to the image data (e.g., list of NumPy arrays or lists representing the labels).

    Methods:
    __init__(self, x, y): Initializes the dataset, storing provided data as NumPy arrays.
    __len__(self): Returns the number of samples in the dataset.
    __getitem__(self, index): Returns a tuple of image and label at the given index as PyTorch tensors, with the image normalized to the range [0, 1].

    Expected Outputs:
    Returns normalized data as PyTorch tensors with on-demand data conversion.
    x_tensor are the image tiles which are normalised and converted into torch tensors whenever __getitem__() is invoked.
    y_tensor are one_hot_encoded labels which are converted into torch tensors whenever __getitem__() is invoked.
    """
...
class TileDatasetLoader(Dataset):
    """
    Handles loading of training tile sets and annotations, with optional data augmentation and dataset balancing.
    ...
    balance_factor: Float multiplier for balancing datasets based on class sizes and oriented on Colonised classes.
    calculate_distribution: Boolean flag to calculate class distribution statistics.

    Methods and Workflow:
    __init__(self, input_files, use_augmentation=False, balance_factor=1.0, calculate_distribution=False):
    Initializes the class with input images, configurations, and optional parameters for augmentation and balancing.
    Calls _prepare_dataset to load and augment data if necessary. Optionally computes class distribution statistics via _prepare_stats.

    _prepare_dataset(input_files, use_augmentation, balance_factor):
    Iterates through each input file, extracting image tiles and annotations. Normalizes image tiles by scaling pixel values to [0, 1].
    Optionally applies data augmentation using specified transformations. Balances the dataset according to the balance_factor parameter.

        if use_augmentation is set to true:
        Utilizes a set of transformations (random flips, color jitter, etc.) to augment specified classes multiple times.
        For these operations tiles and labels are temporarilty converted to torch tensors. Balanced representation is ensured through
        class_augmentations, inidicating the number of augmentation loops per given class in a dictionary.

    _balance_dataset(self, x, y, factor=1.0):
    Balances classes by clipping larger clases to the number of tiles obtained from smaller classes.
    Smaller classes are retrained fully.

    factor (float): The factor to determine the clipping size for classes larger than class 0.
    ...
    - Classes larger than class 0 are clipped to `factor` times class 0's size, but not exceeding the class size.

    _prepare_stats(self):
    Calculates and prints class distribution statistics, as a check and overview that all classes are present within a given dataset.

    __len__(self):
    Returns the number of samples in the dataset.

    __getitem__(self, idx):
    Retrieves data samples and their labels from the dataset, ensuring that images are appropriately scaled.

    Expected Outputs
    dataset = List of tuples containing pairs of image tiles and labels.

    The loader facilitates the generation of a dataset from input images and annotations, outputting:
    Tile Images: Image tiles processed with optional augmentations and balacing, converted into uint8 numpy arrays and stored in a list.
    Labels: Corresponding one-hot encoded labels for each tile, converted into uint8 numpy arrays and stored in an according list.
    ...
    def _prepare_dataset(self, input_files, use_augmentation, balance_factor):
        ...
        if self.balance_factor != 0.0:
            ...
            AmfLog.text(
                f"Balance factor set to: {self.balance_factor}. Generating balanced dataset."
            )
            dataset = list(zip(balanced_tiles, balanced_labels))
        elif self.balance_factor == 0.0:
            # Create unbalanced dataset
            AmfLog.text(
                f"Balance factor set to: {self.balance_factor}. Generating unbalanced dataset."
            )
    ...
    def _prepare_stats(self):
        ...
        if not np.array_equal(uniqueargs_probe_hot_indexes, uniqueargs_headers):
            AmfLog.error(
                "Training data does not represent all classes. Please reconsider training dataset curation",
                AmfLog.ERR_NO_DATA,
            )
...
class StratifiedDatasetSplitter:
    """
    Designed to split a dataset into training and validation sets while maintaining the distribution of class labels.
    This is particularly useful in imbalanced datasets where ensuring the proportional representation of classes in both
    training and validation sets improves model evaluation and training performance.
    Efficient data handling using lists and NumPy ensures that the class can manage datasets of varying size and complexity
    without significant performance degradation.

    Expected Inputs:
    x: Iterable of image data (e.g., list of tiles as NumPy arrays or as simple list).
    y: Iterable of labels corresponding to the image data (e.g., list of NumPy arrays or lists representing the labels).
    val_size (float): Fraction of the dataset to allocate to the validation set. By default, this is set to 0.2 (20% validation).
    random_state (int): Seed used for random number generation to ensure reproducibility of data splits. Default is 42.

    Methods and Workflow:
    __init__(self, x, y, val_size=0.2, random_state=42):
    Takes input features and labels, along with parameters for validation size and random state.
    Calls _split_data to perform the stratified split.

    _split_data(self, val_size, random_state):
    Converts one-hot encoded labels to single integer indices using NumPy operations for efficient computation, including unique labels.
    Subsequently, organises indices of samples into a dictionary keyed by class labels, creating a mapping of data sample locations.
    ...
    - Accumulates indices for training and validation splits based on this shuffled order.

    _get_dataset(self, dataset_type="train"):
    Provides access to data splits by returning features and labels for either the training or validation set, based on the input parameter.
    IMPORTANT: The datasets are returned in their original format, i. e. lists of numpy arrays respectively. Torch tensors are not used.

    Expected Outputs:
    x_train, y_train: Lists of image tiles and one-hot-encoded labels for training, stratified to mirror overall class distribution.
    x_val, y_val: Lists of image tiles and one-hot-encoded labels for validation, stratified to mirror overall class distribution.
    """
...
class TileFilesandData(Dataset):
    ...
    This class effectively handles the extraction and preparation of data needed for machine learning tasks, specifically image-based analysis,
    by organizing and managing the annotations and image data efficiently.

    Expected Inputs:
    input_files: List of file paths to the images used for training. These images should ideally have corresponding annotation files or data.

    Methods and Workflow:
    __init__(self, input_files):
    Uses input_files as input and calls _load_and_process_data method to aggregate image tiles, annotations, and related metadata, filtering out entries without annotations.

    _load_and_process_data(self):
    Loads annotations and settings for each provided image file unsing import_annotations and import_settings. Filters the image files to ensure only those with annotations are processed.
    Calls _process_dataset to process each image and extract tiles and metadata.

    _process_dataset(self, dataset):
    Iterates through a filtered datasetimage file, accessing configurations and annotations. Tiles are extracted based on annotations.
    Collects tile data, labels, file names, and their row/column position data into lists and compiles them into a tuple containing tiles, labels, filenames, row indices, and column indices.

    Expected Outputs:
    x (torch.FloatTensor): Normalized tiles as a PyTorch tensor, with pixel values scaled to [0, 1].
    ...
    def _load_and_process_data(self, is_bald_folder=False, is_unlabelled=False):
        ...
        if is_unlabelled:
            ...
            print(
                f"[{AmfConfig.invite()}] {len(filtered_dataset)} images in filtered dataset."
            )
...
class FixMatchLoader:
    ...
    def load_val_dataset(self, input_files: list, root: str):
        ...
        def process_dataset(dataset: list = None):
            ...
            for path, annots in dataset:
                ...
                # get the max examples based on the max number of examples in the minority class
                max_examples = min(
    ...
    def load_train_dataset(self, input_files, num_labelled, args):
        """
        Loads training tile set and their corresponding annotations, and stores the file name for each tile.

        :param input_files: List of input images to use for training.
        :return: Numpy arrays containing tiles, one-hot encoded annotations, and list of file names.
        :rtype: tuple
        """
        ...
        # This function assumes images have been properly sampled and labels extracted beforehand.
...
def import_annotations(path, is_bald_folder=False):
    ...
    try:
        ...
        if AmfConfig.get("use_db"):
            ...
            # If question does not exist then do not error when dropping (for legacy CSVs)
            if "Question" in output.columns:
                print(f"Dropping questions from annotations for {image_name}")
                output.drop("Question", axis=1, inplace=True, errors="ignore")
...
def print_table_header():
    ...
    if AmfConfig.get("level") == 1:
        ...
        print(
            "Filename\tImage size (px)\tTile size (px)\tAM+\tN-\tX\tU\tDSE\tH\tDiscarded"
        )
...
def print_image_stats(path, image, config, annots, discarded):
    ...
    if AmfConfig.get("level") == 1:
        ...
        a = annots["AMColonised"].sum()
        m = annots["Uncolonised"].sum()
        x = annots["Background"].sum()
        u = annots["Unreadable"].sum()
        d = annots["DSE"].sum()
        h = annots["Hybrid"].sum()

        print(
            f"{base}\t{w}x{h}\t{edge}\t{a}\t{m}\t{x}\t{u}\t{d}\t{h}\t{discarded}",
            flush=True,
        )

    else:

        a = annots["Arbuscule"].sum()
        v = annots["Vesicle"].sum()
        hy = annots["Hyphopodium"].sum()
        i = annots["Hypha"].sum()

        print(f"{base}\t{w}x{h}\t{edge}\t{a}\t{v}\t{hy}\t{i}", flush=True)


def get_image_names_without_extension(file_paths):
    image_names = []
    for path in file_paths:
        # Get the basename of the path and split at the last dot to remove the extension
        name = os.path.basename(path)  # Extracts the file name from the path
        name_without_extension = os.path.splitext(name)[0]  # Removes the file extension
