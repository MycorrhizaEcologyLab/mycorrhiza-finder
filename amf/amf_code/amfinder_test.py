...
# This is currently not used as only max vote is used for context
def apply_aggregation_rules(
    tile_predictions, class_names_map
):  # This is only for reference now as it was empirically discovered that maxvote highly outperforms this complex aggregation method
    """
    Apply aggregation rules from amfinder_convert_tile_size to determine the final class.

    Args:
        tile_predictions: List of class indices for the tiles (including the central tile).
        class_names_map: Dict mapping class indices to class names.

    Returns:
        Final class index after applying the aggregation rules.
    """
    # Convert class indices to class names
    class_instances = [
        class_names_map.get(idx, f"unknown_{idx}") for idx in tile_predictions
    ]

    # Count occurrences of each class
    class_counts = {}
    for class_name in set(class_instances):
        class_counts[class_name] = class_instances.count(class_name)
    ...
    majority_ratio = class_counts[majority_class] / total_tiles

    # Initialize result dictionary
    result = {name: 0 for name in set(class_instances)}

    # If all tiles have the same class, use that class
    if majority_ratio == 1.0:
        result[majority_class] = 1
    else:
        # Calculate ratios
        ratios = {name: count / total_tiles for name, count in class_counts.items()}

        # Apply aggregation rules based on majority class
        if majority_class == "AMColonised":
            if ratios.get("DSE", 0) > 0.0 or ratios.get("Hybrid", 0) > 0.0:
                result["Hybrid"] = 1
            else:
                result["AMColonised"] = 1

        elif majority_class == "Uncolonised":
            if ratios.get("AMColonised", 0) > 0.0 and (
                ratios.get("DSE", 0) > 0.0 or ratios.get("Hybrid", 0) > 0.0
            ):
                result["Hybrid"] = 1
            elif (
                ratios.get("AMColonised", 0) > 0.0
                and ratios.get("DSE", 0) == 0.0
                and ratios.get("Hybrid", 0) == 0.0
            ):
                result["AMColonised"] = 1
            elif (
                ratios.get("AMColonised", 0) == 0.0
                ...
                result["Uncolonised"] = 1

        elif majority_class == "Background":
            if ratios.get("AMColonised", 0) > 0.0 and (
                ratios.get("DSE", 0) > 0.0 or ratios.get("Hybrid", 0) > 0.0
            ):
                result["Hybrid"] = 1
                ...
                and ratios.get("DSE", 0) == 0.0
                and ratios.get("Hybrid", 0) == 0.0
                and ratios.get("Uncolonised", 0) > 0.0
            ):
                result["Uncolonised"] = 1
            elif (
                ratios.get("AMColonised", 0) == 0.0
                ...
                and ratios.get("Hybrid", 0) == 0.0
                and ratios.get("Uncolonised", 0) == 0.0
                and ratios.get("Unreadable", 0) > 0.0
            ):
                result["Unreadable"] = 1
            else:
                result["Background"] = 1

        elif majority_class == "Unreadable":
            if ratios.get("Unreadable", 0) > 0.5:
                result["Unreadable"] = 1
            else:
                if ratios.get("AMColonised", 0) > 0.0 and (
                    ratios.get("DSE", 0) > 0.0 or ratios.get("Hybrid", 0) > 0.0
        ...
        elif majority_class == "DSE":
            if ratios.get("AMColonised", 0) > 0.0 or ratios.get("Hybrid", 0) > 0.0:
                result["Hybrid"] = 1
            else:
                result["DSE"] = 1

        elif majority_class == "Hybrid":
            result["Hybrid"] = 1

        else:
            # Default case: use majority class
            result[majority_class] = 1
    ...
    # If final_class is not set, use majority_class
    if final_class is None:
        final_class = majority_class

    # Return the class index corresponding to the final class name
    for idx, name in class_names_map.items():
        if name == final_class:
            return idx

    # Fallback: return the first class index in the predictions
...
def get_test_results(
            ...
            for file_path, indices_with_coords in file_to_indices.items():
                ...
                for (
                    ...
                    if num_tiles > 0:
                        ...
                        # final_class = apply_aggregation_rules(central_tile_with_neighborhood_preds, class_names_map) # complex aggregation rules
    ...
    # metrics.get_example_predictions()     # This will not work on small datasets and non-trained models. TODO: Review function and integrate fully with appropriate model.
